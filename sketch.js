...
function setup() {
  ...
  owow3dlnk.position(windowWidth/3, 300);
  ...
  owowclsclnk.position(windowWidth/3, 300);
...
function draw() {
  ...
  if (play == true) {
    ...
    if (show < 40) {
      flash(windowWidth/4 +30, 80, 1);
    } else {
      image(sl1[rand], windowWidth/4 + 30, 80, px, px);
    }

    if (show < 50) {
      flash(windowWidth/4 + 180, 80, 2);
    } else {
      image(sl2[rand1], windowWidth/4 + 180, 80, px, px);
    }

    if (show < 60) {
      flash(windowWidth/4 + 330, 80, 3);
    } else {
      image(sl3[rand2], windowWidth/4 + 330, 80, px, px);
    }
    ...
    if (show > 61) {
      if (rand1 == rand) {
        ...
        fill("red");
        text("You lost! Try Again", windowWidth/4 + 10, 280);
      }
    }
  }
  if (win) {
  ...
  if (gif_trig) {
    image(spinwow, windowWidth/3, 30, 350, 250);
...
function reset_slots() {
  ...
  textSize(50);
  fill("black");
  text("owow slot machine", windowWidth/4, 60);
  przwarning.show();
  gif_trig = false;
  choose_rand();
}
function keyPressed() {
  if (key === "p") {
    if (prz) {
      r_button.hide();
      button.hide();
      image(prize_list, windowWidth/4, 80, 500, 300);
      play = false;
      prz = false;
...
function windowResized() {
  ...
  if (currentAspectRatio > aspectRatio) {
    // Wider than base aspect ratio, scale height and add horizontal bars
    scaleFactor = windowHeight / baseHeight;
    let scaledWidth = baseWidth * scaleFactor;
    let xOffset = (windowWidth - scaledWidth) / 2;
    // Adjust drawing coordinates based on xOffset and scaleFactor
  } else {
    // Taller than base aspect ratio, scale width and add vertical bars
    scaleFactor = windowWidth / baseWidth;
    let scaledHeight = baseHeight * scaleFactor;
    let yOffset = (windowHeight - scaledHeight) / 2;
    // Adjust drawing coordinates based on yOffset and scaleFactor
  }
  // Adjust canvas size based on scaling and offset
}